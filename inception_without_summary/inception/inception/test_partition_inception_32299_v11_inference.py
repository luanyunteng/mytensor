...
path = list()
filereader = open('result_by_graph_1_minibatch32_299_logits', 'r')
...
print('read path')
pathtime = list()
filereader = open('time_by_graph_1_minibatch32_299_logits', 'r')
line = filereader.readline()
while line:
...
q = list()
logWriter = open('log_v11.txt', "a")
for curdeep in range(1, len(path)-1):
    print("cur deep: ", curdeep)
    logWriter.write('**********************************************\n')
    logWriter.write('cur deep: ' + str(curdeep) + 'pathtime: ' + str(pathtime[curdeep]) + '\n')
    allpre = dict()
    for pre in dic[path[curdeep]]:#遍历curdeep层节点的前置节点
        if pre not in pathset:
            allpre[pre] = get_subgraph(pre)
    allpre_time = list()
    allpre_partition = dict()
    graph_0_allpre_partition = dict()
    
    for key in allpre:
        
        replacement_list = allpre[key].copy()
        ...
        allpre_partition[key] = replacement_list.copy()
        ...
        for x in replacement_list:
            ...
            current_write += 1
        fileWriter.close()
        #print(path[curdeep], ':    ', replacement_list)
        ...
        graph_0_allpre_partition[key] = graph_0_replacement_list.copy()
        ...
        fileWriter.close()
        #print('************************')
        #print(path[curdeep], ': ',graph_0_replacement_list)
        ...
        for i in range(100):
            sess.run(op)
        allpre_time.append(((time.clock() - start) * 1000 / 100, key))
        print("key: ", key, "    time: ", (time.clock() - start) * 1000 / 100)
        ...
        with open('/home/lyt/static_analysis/replacement/replace_init.txt', 'r') as old_file:
            with open('/home/lyt/static_analysis/replacement/replace_init.txt', 'r+') as new_file:
                ...
                new_file.seek(seek_point, 0)
                new_file.truncate()       
                graph_0_replacement_num -= graph_0_current_write
    #''' 
    allpre_time = sorted(allpre_time)
    allpre_time_copy = [(k,v) for (k,v) in allpre_time if k<(pathtime[curdeep]- reducetime)]
    allpre_time = allpre_time_copy
    cur_min_time = pathtime[curdeep] - reducetime
    if len(allpre_time)>0:
        dur, key = allpre_time[0]
        cur_min_time = dur
        fileWriter = open('/home/lyt/static_analysis/replacement/replace_train.txt', 'a')
        for x in allpre_partition[key]:
            data = op_information[x].split('[')[3]
            data = data.split(']')[0]
            fileWriter.write(str(data) + ' ' + str(gpu_id) + '\n')
            replacement_num += 1
        fileWriter.close()
        fileWriter = open('/home/lyt/static_analysis/replacement/replace_init.txt', 'a')
        for x in graph_0_allpre_partition[key]:
            data = graph_0_information[x].split('[')[3]
            data = data.split(']')[0]
            fileWriter.write(str(data) + ' ' + str(gpu_id) + '\n')
            graph_0_replacement_num += 1
        ...
        allpre_time.remove(allpre_time[0])
        print('cur_min_time:                                  ',cur_min_time)
        print('we find a good partition. key: ' + str(key) + '    time: ' + str(cur_min_time))
        #for k in dic:      
            #dic[k] = dic[k] - allpre_partition[key]

    for dur, key in allpre_time:
        print(dur, ',                                          ', key)
        fileWriter = open('/home/lyt/static_analysis/replacement/replace_train.txt', 'a')
        current_write = 0
        graph_0_current_write = 0
        ...
        for x in graph_0_allpre_partition[key]:
            ...
            graph_0_current_write += 1
        fileWriter.close()
        #print('partition:   ', allprepartition[key])
        #print('-------------------------')
        #print('graph_0_partition:    ', graph_0_allpre_partition[key])
        op_name = op_information[path[curdeep]].split('[')[3].split(']')[0]
        op = tf.get_default_graph().get_operation_by_name(op_name)
        sess = tf.Session()
        os.rename('/home/lyt/static_analysis/replacement/replace_init.txt', '/home/lyt/static_analysis/replacement/replace.txt')
        ...
        for i in range(100):
            sess.run(op)
        #print("key: ", key, "    time: ", (time.clock() - start) * 1000 / 100)
        cur_dur = (time.clock() - start) * 1000 / 100
        os.rename('/home/lyt/static_analysis/replacement/replace.txt', '/home/lyt/static_analysis/replacement/replace_train.txt')
        if cur_dur >= cur_min_time:
            print(cur_dur,'>=',cur_min_time)
            with open('/home/lyt/static_analysis/replacement/replace_train.txt', 'r') as old_file:
                with open('/home/lyt/static_analysis/replacement/replace_train.txt', 'r+') as new_file:
                    current_line = 0
                    while current_line < replacement_num - current_write:
            ...
            cur_min_time = cur_dur
            print('cur_min_time changed:  ', cur_min_time)
            logWriter.write('we add a partition: ' + str(key) + '    time: ' + str(cur_min_time) + '\n')
            #for k in dic:
                #dic[k] = dic[k] - allpre_partition[key]

    reducetime = pathtime[curdeep] - cur_min_time
    #'''
    ...
    if len(replacement_list) > 0:
        ...
        #deepWriter = open('/home/lyt/log/deep' + str(curdeep) + '.txt', 'a')
        for x in replacement_list:
            data = op_information[x].split('[')[3]
            data = data.split(']')[0]
            #添加graph_0需要replace的节点
            ...
            fileWriter.write(str(data) + ' ' + str(gpu_id) + '\n')
            #deepWriter.write(op_information[x])
    ...
    current_write = 0
    fileWriter = open('/home/lyt/static_analysis/replacement/replace_train.txt', 'a')
    record_replacement = set()
    for key in allpre_partition:
        record_replacement = record_replacement | allpre_partition[key]
        for x in allpre_partition[key]:
            data = op_information[x].split('[')[3]
            data = data.split(']')[0]
            fileWriter.write(str(data) + ' ' + str(gpu_id) + '\n')
            ...
            replacement_num += 1
    fileWriter.close()
    print(record_replacement)
    graph_0_current_write = 0
    fileWriter = open('/home/lyt/static_analysis/replacement/replace_init.txt', 'a')
    for key in graph_0_allpre_partition:
        for x in graph_0_allpre_partition[key]:
            data = graph_0_information[x].split('[')[3]
            data = data.split(']')[0]
            fileWriter.write(str(data) + ' ' + str(gpu_id) + '\n')
    ...
    for i in range(100):
        sess.run(op)
    #print("key: ", key, "    time: ", (time.clock() - start) * 1000 / 100)
    dur = (time.clock() - start) * 1000 / 100
    os.rename('/home/lyt/static_analysis/replacement/replace.txt', '/home/lyt/static_analysis/replacement/replace_train.txt')

    if dur >= (pathtime[curdeep] - reducetime):

        print('                               no reduce time')
        logWriter.write('all partition is not good\n')
        #logWriter.write('no reduce time.  ' + 'curtime: ' + str(dur) + '          pathtime: ' + str(pathtime[curdeep]) + '\n')
        print('curtime: ', dur, 'ms', '           pathtime: ', pathtime[curdeep], 'ms')
        with open('/home/lyt/static_analysis/replacement/replace_train.txt', 'r') as old_file:
            with open('/home/lyt/static_analysis/replacement/replace_train.txt', 'r+') as new_file:
                current_line = 0
        ...
        #print("key: ", key, "    time: ", (time.clock() - start) * 1000 / 100)
        dur = (time.clock() - start) * 1000 / 100
        print('current partition time: ', dur, 'reducetime: ',reducetime)
        logWriter.write('current partition time: ' + str(dur) + '    reducetime: ' + str(reducetime) + '\n')
        os.rename('/home/lyt/static_analysis/replacement/replace.txt', '/home/lyt/static_analysis/replacement/replace_train.txt')
        # we can partition it so we need to change the topo
    else:
        reducetime = pathtime[curdeep] - dur 
        print('opname: ', op_name, 'runningtime: ', dur, 'ms.', 'reducetime:        ', reducetime, 'ms')
        logWriter.write('all partition is good.    reducetime: ' + str(reducetime) + '\n' )
        for k in dic:      
            dic[k] = dic[k] - record_replacement
    print('curdeep reduce time: ', reducetime)
logWriter.close()
'''
sess = tf.Session()
