...
with tf.Session() as sess:
    ...
    def get_run_time(op_id, sess):
        ...
        try:
            ...
            for i in range(100):
                sess.run(op)
            res_duration = (time.clock() - start) * 1000 / 100
    ...
    print(path)
    filewriter = open('result_by_graph_1_minibatch32_299_logits','w')
    for x in path:
        filewriter.write(lines[x])
    filewriter.close()
    filewriter = open('time_by_graph_1_minibatch32_299_logits', 'w')
    for x in pathtime:
