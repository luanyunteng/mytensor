...
Status Partition(const PartitionOptions& opts, Graph* g,
  ...
  printf("in Partition()\n");
  /*
  string edge_name = "edge_10213_images";
  for(Edge* e: g->edges()){
    if(e->name().data() == edge_name){
      printf("src node:       %s\n", e->src()->name().data());
      printf("dst node:       %s\n", e->dst()->name().data());
    }
  }
  printf("end edge!\n");
  */
  bool config_graph_placement = true;
  if (config_graph_placement) {
    printf("read replacement\n");
    std::ifstream infile("/home/lyt/static_analysis/replacement/replace.txt");
    string op_name;
    string gpu_id;
    Node* curNode = nullptr;
    while (infile >> op_name  >> gpu_id) {
        //printf("op_name:%s, gpuid:%s\n",op_name.data(), gpu_id.data());
        int totalnum = g->num_node_ids();
        for(int i= 0;i<totalnum;i++){
          Node* node = g->FindNodeId(i);
          if(node->name().data() == op_name){
            curNode = node;
            break;
          }
        }
    //printf("totalnum:%d\n", totalnum);
    if (curNode != nullptr){     
      string d_name = curNode->assigned_device_name();
      size_t pos = d_name.rfind("gpu:");
      if (pos != string::npos){
        //d_name = d_name.substr(0,pos+4) + gpu_id;
        //node->set_assigned_device_name(d_name);
        d_name = d_name.substr(0,pos+4) + gpu_id;        
        curNode->set_assigned_device_name(d_name);
        //printf("nodename:                                      %s\n", curNode->name().data());
      }
        curNode = nullptr; 
    }
    ...
    //Node* node = g->FindNodeId(1079);
    //string d_name = node->assigned_device_name();
      //size_t pos = d_name.rfind("gpu:");
    //d_name = d_name.substr(0,pos+4) + "1";
    //node->set_assigned_device_name(d_name);
    //printf("%d node replacemennt!\n", 1079);
  ...
  if (dump_info){
    string dir = "/home/lyt/static_analysis/result/";
    {
  ...
  if (dump_info) {
    ...
    for (int i=0;i<nverts;i++)
    {
      ...
      const Edge* out_edge = nullptr;
      EdgeSet::const_iterator it = node->out_edges().begin();
      while(it !=node->out_edges().end()){
        out_edge = *it;
        Node* dst_node = out_edge->dst();
        if(out_edge->IsControlEdge()){
            fprintf(dump_file_graph, "{%d,%d} ", 1, dst_node->id());    
        }else{
            int des_input = out_edge->dst_input();
            DataType dt = dst_node->input_type(des_input);
            fprintf(dump_file_graph, "{%d,%d} ", dt, dst_node->id());
        }
        it++;
      }
